...
public class TinySearchEngine implements TinySearchEngineBase {
    ...
    public void insert(Word word, Attributes atrbts) {
        ...
        boolean wordPreviouslyAdded = false;
        
        if (insertAt < this.words.size()) {
            wordPreviouslyAdded = this.words.get(insertAt).word.equals(word.word);
        }
        
        if (wordPreviouslyAdded) {
    ...
    private int sortedPosition (Word word) {
        ...
        while (lo <= hi) {
            int mid = lo + (hi - lo) / 2;
            int comparison = word.word.compareTo(this.words.get(mid).word);
